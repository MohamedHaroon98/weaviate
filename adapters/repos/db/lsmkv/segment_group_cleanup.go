...
type segmentCleaner interface {
	close() error
	cleanupOnce(cyclemanager.ShouldAbortCallback) (bool, error)
}
...
func (c *segmentCleanerImpl) findCandidate() (int, onCleanupCompletedFunc, error) {
	noCandidate := -1
...
func (c *segmentCleanerImpl) storeNextAllowed(ts int64) error {
	if err := c.db.Update(func(tx *bolt.Tx) error {
		...
		return b.Put(cleanupMetaKeyTsOldest, bufV)
	}); err != nil {
